<?php
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://bsn.by/register_enter.php"); /* Переходим на страницу, на которой нужно пройти процедуру авторизации */
...
$result = curl_exec($ch);
curl_setopt($ch, CURLOPT_URL, "https://bsn.by/otpravlen.php"); /* Теперь заходите на сайт как авторизованный пользователь – в данном случае нужно вместо example.com указать URL страницы, на которую может попасть только авторизованный пользователь */
